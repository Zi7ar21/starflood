...
class BarnesHut {
	public:
		...
		void Build() {
			...
			// Warning: May cause an unrecoverable memory leak due to infinite tree recursion for some reason
			// Open htop and get your [Ctrl]+[C] fingers ready before it hits swap...
			...
			for(int i = 0U; i < node.size(); i++) {
				// divide the sum of position times mass by the number of masses (final step in calculating center of mass)
				node[i].mass_x /= (node[i].mass > 0.0) ? (double)node[i].mass : 1.0;
				node[i].mass_y /= (node[i].mass > 0.0) ? (double)node[i].mass : 1.0;
			}
		...
		void UpdateAcceleration() {
			const Real theta = (Real)0.1;

			for(int i = 0U; i < (2U * N); i++) a[i] = (Real)0;
			...
			for(int i = 0U; i < N; i++) {
				...
				while(!to_visit.empty()) {
					...
					Real d2 = (dx*dx)+(dy*dy);
					if(d2 < (Real)(0.001 * 0.001)) continue; // too close
					Real d1 = sqrtf(d2);

					if((node[cur_node].particles_contained == 1) || ((width / d1) < theta) || (node[cur_node].child1 == -1 && node[cur_node].child2 == -1 && node[cur_node].child3 == -1 && node[cur_node].child4 == -1)) {
						// calculate forces
...
int main(int argc, char** argv) {
	...
	unsigned int ImageSizeX = 512u;
	unsigned int ImageSizeY = 512u;
	...
	Buffer2D<RGBA32F> image(ImageSizeX, ImageSizeY); // Create an image buffer (we will render to this later)

	rng_state_t initial_conditions_rng_state; // rng state for generating initial conditions

	//simulation.Initialize();

	size_t m_buffer_size = (size_t)N * (size_t)1 * sizeof(Real); // mass buffer size
	size_t r_buffer_size = (size_t)N * (size_t)2 * sizeof(Real); // position buffer size
	size_t v_buffer_size = (size_t)N * (size_t)2 * sizeof(Real); // velocity buffer size
	size_t a_buffer_size = (size_t)N * (size_t)2 * sizeof(Real); // acceleration buffer size
	size_t t_buffer_size = m_buffer_size + r_buffer_size + v_buffer_size + a_buffer_size; // total buffer size

	#if STARFLOOD_VERBOSITY > 0
	printf("m_buffer_size: %zu\nr_buffer_size: %zu\nv_buffer_size: %zu\na_buffer_size: %zu\n", m_buffer_size, r_buffer_size, v_buffer_size, a_buffer_size);
	#endif

	printf("Allocating %zu B...", t_buffer_size);
	fflush(stdout);

	Real* m = (Real*)malloc(m_buffer_size); // mass buffer
	...
	Real* r = (Real*)malloc(r_buffer_size); // position buffer

	if(r == NULL) {
		perror(" malloc() returned NULL when allocating r! Error");

		return EXIT_FAILURE;
	}

	Real* v = (Real*)malloc(v_buffer_size); // velocity buffer
	...
	Real* a = (Real*)malloc(a_buffer_size); // acceleration buffer
	...
	//#pragma omp simd
	for(unsigned int i = 0u; i < N; i++) m[i] = (Real)((double)10.0 / (double)N);

	initial_conditions_rng_state.x = (uint32_t)1; // reseed the initial condition random number generator

	// initialize particle position
	//for(unsigned int i = 0; i < (2 * N); i++) r[i] = 1.0 * ((2.0 * ((long double)std::rand() / (long double)RAND_MAX)) - 1.0);
	for(unsigned int i = 0U; i < (2U * N); i++) r[i] = (Real)(1.0 * (2.0f * (urand1(&initial_conditions_rng_state) - 0.5f)));

	initial_conditions_rng_state.x = (uint32_t)2; // reseed the initial condition random number generator

	// initialize particle velocity
	//for(unsigned int i = 0; i < (2 * N); i++) v[i] = 0.01 * ((2.0 * ((long double)std::rand() / (long double)RAND_MAX)) - 1.0);
	for(unsigned int i = 0U; i < (2U * N); i++) v[i] = (Real)(0.01 * (2.0f * (urand1(&initial_conditions_rng_state) - 0.5f)));
	...
	//}
	//for(unsigned int i = 0U; i < N; i++) {
	//	v[2 * i + 0] =  0.1 * r[2 * i + 1] / (r[2 * i + 0]*r[2 * i + 0]+r[2 * i + 1]*r[2 * i + 1]+1.0);
	//	v[2 * i + 1] = -0.1 * r[2 * i + 0] / (r[2 * i + 0]*r[2 * i + 0]+r[2 * i + 1]*r[2 * i + 1]+1.0);
	//}

	//for(unsigned int i = 0; i < (2U * N); i++) a[i] = (Real)0;

	//UpdateAcceleration(N, m, r, v, a); // update acceleration

	//std::srand(1); std::rand();
	//initial_conditions_rng_state.x = (uint32_t)1;
	//for(unsigned int i = 0U; i < (2U * N); i++) if(r[i] != r[i]) r[i] = 1.0 * ((1.0 * ((long double)std::rand() / (long double)RAND_MAX)) - 1.0);
	//for(unsigned int i = 0U; i < (2U * N); i++) if(r[i] != r[i]) r[i] = (Real)(1.0 * (2.0f * (urand1(&initial_conditions_rng_state) - 0.5f)));

	//std::srand(2); std::rand();
	//initial_conditions_rng_state.x = (uint32_t)2;
	//for(unsigned int i = 0U; i < (2U * N); i++) if(v[i] != v[i]) v[i] = 0.1 * ((0.01 * ((long double)std::rand() / (long double)RAND_MAX)) - 1.0);
	//for(unsigned int i = 0U; i < (2U * N); i++) if(v[i] != v[i]) v[i] = (Real)(0.01 * (2.0f * (urand1(&initial_conditions_rng_state) - 0.5f)));

	//for(unsigned int i = 0U; i < (2U * N); i++) if(a[i] != a[i]) a[i] = (Real)0;

	BarnesHut barneshut((Real)-1.0, (Real)-1.0, (Real)1.0, (Real)1.0);
	...
	for(unsigned int i = 1u; i < N; i++) {
		min_x = fmin(min_x, r[2 * i + 0]);
		max_x = fmax(max_x, r[2 * i + 0]);
		min_y = fmin(min_y, r[2 * i + 1]);
		max_y = fmax(max_y, r[2 * i + 1]);
	}
	...
	for(int n = 1; n < 1200; n++) {
		...
		//#pragma omp simd
		for(unsigned int i = 0U; i < (2U * N); i++) v[i] += (Real)0.5 * dt * a[i]; // 1/2 kick
		//v = v + (0.5 * dt * a); // 1/2 kick

		//for(unsigned int i = 0; i < (2 * N); i++) v[i] *= 0.99;

		//#pragma omp simd
		for(unsigned int i = 0U; i < (2U * N); i++) r[i] += dt * v[i]; // drift
		//r = r + (dt * v); // drift
	...
	#if STARFLOOD_VERBOSITY > 0
	printf("Freeing m buffer (%zu B)...", m_buffer_size);
	fflush(stdout);
	#else
	printf("Freeing %zu B...", t_buffer_size);
	fflush(stdout);
	#endif
	...
	#if STARFLOOD_VERBOSITY > 0
	printf(" Done!\nFreeing r buffer (%zu B)...", r_buffer_size);
	fflush(stdout);
	#endif
	...
	#if STARFLOOD_VERBOSITY > 0
	printf(" Done!\nFreeing v buffer (%zu B)...", v_buffer_size);
	fflush(stdout);
	#endif
	...
	#if STARFLOOD_VERBOSITY > 0
	printf(" Done!\nFreeing a buffer (%zu B)...", a_buffer_size);
	fflush(stdout);
	#endif
